...
torchrun  --nproc_per_node=2 --master_port=$PORT finetune.py   \
...
do
    torchrun --nproc_per_node=2 --master_port=$PORT evaluate.py  --model LLaMA-7B --load_8bit  --adapter LoRA --dataset $task --base_model 'yahma/llama-7b-hf' --lora_weights './trained_models/llama-7b-lora-math/'     # specify the base model
